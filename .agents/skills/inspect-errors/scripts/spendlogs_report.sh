  ...
  SELECT *,
         CASE
           ...
           WHEN message ILIKE '%maximum context length%'
             ...
             OR message ILIKE '%max_completion_tokens=%greater than%'
           ...
           WHEN message ILIKE '%validation errors%'
             ...
             OR message ILIKE '%tool_choice%incompatible%'
             OR message ILIKE '%validation error for%'
             OR message ILIKE '%unsupported reasoning effort%'
             OR message ILIKE '%supports text and image content only%'
             OR message ILIKE '%image file is truncated%'
             OR message ILIKE '%resolvable tool name%'
             OR message ILIKE '%custom tool arguments must include a content field%'
             OR message ILIKE '%text content is empty%'
             OR message ILIKE '%stream options can only be defined%' THEN 'client_request_format'
           ...
           WHEN message ILIKE '%no api key passed in%'
             OR message ILIKE '%expired key%'
           ...
           WHEN message ILIKE '%connection closed%'
             ...
             OR message ILIKE '%server disconnected%'
             OR message ILIKE '%response payload is not completed%' THEN 'backend_connection_closed'
           WHEN error_code = '429'
