...
Also adjusts matching EndUser cumulative spend when no end-user budget is configured.
Skips records written or completed in the last 60 seconds. The proxy stays running.
...
RUNTIME_CONFIG_OUTPUT=$("${COMPOSE[@]}" exec -T litellm python -c \
	'import json
import yaml
...
print(json.dumps(ROUTE_ENDPOINT_MAPPING, separators=(",", ":")))
print(settings.get("max_end_user_budget_id") or "")')
mapfile -t RUNTIME_CONFIG <<<"$RUNTIME_CONFIG_OUTPUT"
ROUTE_ENDPOINT_MAPPING="${RUNTIME_CONFIG[0]}"
MAX_END_USER_BUDGET_ID="${RUNTIME_CONFIG[1]:-}"

PSQL=(
	"${COMPOSE[@]}"
	exec -T db
	psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"
	-q -v ON_ERROR_STOP=1 -v "route_endpoint_mapping=$ROUTE_ENDPOINT_MAPPING" -v "max_end_user_budget_id=$MAX_END_USER_BUDGET_ID" -P pager=off
)
...
print_staging_sql() {
	cat <<'SQL'
CREATE TEMP TABLE _litellm_cleanup_cutoff ON COMMIT DROP AS
SELECT (now() AT TIME ZONE 'UTC') - interval '60 seconds' AS cutoff;

CREATE TEMP TABLE _litellm_recent_usage_groups ON COMMIT DROP AS
SELECT DISTINCT api_key, ("startTime" AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Shanghai')::date::text AS date
FROM "LiteLLM_SpendLogs"
WHERE "endTime" >= (SELECT cutoff FROM _litellm_cleanup_cutoff)
   OR updated_at >= (SELECT cutoff FROM _litellm_cleanup_cutoff)
UNION SELECT api_key, date FROM "LiteLLM_DailyUserSpend" WHERE updated_at >= (SELECT cutoff FROM _litellm_cleanup_cutoff)
UNION SELECT api_key, date FROM "LiteLLM_DailyTeamSpend" WHERE updated_at >= (SELECT cutoff FROM _litellm_cleanup_cutoff)
UNION SELECT api_key, date FROM "LiteLLM_DailyTagSpend" WHERE updated_at >= (SELECT cutoff FROM _litellm_cleanup_cutoff)
UNION SELECT api_key, date FROM "LiteLLM_DailyEndUserSpend" WHERE updated_at >= (SELECT cutoff FROM _litellm_cleanup_cutoff);
...
CREATE TEMP TABLE _litellm_cleanup_runtime_settings(default_end_user_budget_id text) ON COMMIT DROP;
INSERT INTO _litellm_cleanup_runtime_settings(default_end_user_budget_id)
VALUES (NULLIF(:'max_end_user_budget_id', ''));
...
WHERE s.status = 'failure'
  AND s."endTime" < (SELECT cutoff FROM _litellm_cleanup_cutoff)
  AND s.updated_at < (SELECT cutoff FROM _litellm_cleanup_cutoff);
...
CROSS JOIN LATERAL (
  ...
  WHERE s."startTime" >= d.date::timestamp - interval '1 day'
    ...
    AND s.updated_at < (SELECT cutoff FROM _litellm_cleanup_cutoff)
    AND NOT EXISTS (
      SELECT 1 FROM _litellm_recent_usage_groups r
      WHERE r.api_key = s.api_key
        AND r.date = (s."startTime" AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Shanghai')::date::text
    )
) s
...
CREATE TEMP TABLE _litellm_deleted_end_user_spend ON COMMIT DROP AS
WITH deleted_request_ids AS MATERIALIZED (
  SELECT request_id FROM _litellm_failure_spendlogs
  UNION
  SELECT request_id FROM _litellm_usage_noise_spendlogs
)
SELECT s.end_user, e.budget_id, SUM(s.spend) AS spend
FROM "LiteLLM_SpendLogs" s
JOIN deleted_request_ids d USING (request_id)
JOIN "LiteLLM_EndUserTable" e ON e.user_id = s.end_user
WHERE s.end_user IS NOT NULL
  AND s.end_user <> ''
GROUP BY s.end_user, e.budget_id;
...
UNION ALL
SELECT 'LiteLLM_EndUserTable spend counters to debit', COUNT(*)
FROM _litellm_deleted_end_user_spend
WHERE spend <> 0
UNION ALL
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM _litellm_deleted_end_user_spend
    WHERE budget_id IS NOT NULL AND spend <> 0
  ) THEN
    RAISE EXCEPTION 'Deleted spend logs target end users with an explicit budget; aborting cleanup';
  END IF;
  IF (
    EXISTS (
      SELECT 1 FROM _litellm_cleanup_runtime_settings
      WHERE default_end_user_budget_id IS NOT NULL
    )
    OR EXISTS (
      SELECT 1 FROM "LiteLLM_Config"
      WHERE param_name = 'litellm_settings'
        AND NULLIF(param_value->>'max_end_user_budget_id', '') IS NOT NULL
    )
  ) AND EXISTS (
    SELECT 1 FROM _litellm_deleted_end_user_spend
    WHERE budget_id IS NULL AND spend <> 0
  ) THEN
    RAISE EXCEPTION 'A default end-user budget is configured; aborting cleanup';
  END IF;
  FOR target IN
    SELECT end_user, spend
    FROM _litellm_deleted_end_user_spend
    WHERE budget_id IS NULL AND spend <> 0
    ORDER BY end_user
  LOOP
    SELECT spend, budget_id
    INTO current_spend, current_budget_id
    FROM "LiteLLM_EndUserTable"
    WHERE user_id = target.end_user
    FOR UPDATE;
    IF current_spend IS NULL THEN
      RAISE EXCEPTION 'End-user row disappeared during cleanup; aborting cleanup';
    END IF;
    IF current_budget_id IS NOT NULL THEN
      RAISE EXCEPTION 'End-user budget changed during cleanup; aborting cleanup';
    END IF;
    IF current_spend - target.spend < -1e-12 THEN
      RAISE EXCEPTION 'End-user spend is lower than deleted spend logs; aborting cleanup';
    END IF;
    UPDATE "LiteLLM_EndUserTable"
    SET spend = CASE WHEN ABS(spend - target.spend) <= 1e-12 THEN 0 ELSE spend - target.spend END
    WHERE user_id = target.end_user;
    GET DIAGNOSTICS updated_rows = ROW_COUNT;
    IF updated_rows <> 1 THEN
      RAISE EXCEPTION 'End-user row disappeared during cleanup; aborting cleanup';
...
SET spend = CASE WHEN successful_requests = 0 THEN 0 ELSE spend END,
    ...
    failed_requests = 0
WHERE failed_requests > 0
  AND updated_at < (SELECT cutoff FROM _litellm_cleanup_cutoff)
  AND NOT EXISTS (SELECT 1 FROM _litellm_recent_usage_groups r WHERE r.api_key = d.api_key AND r.date = d.date);
