...
# Single fetch of the upstream base so the delta-based gates below share one network round
...
lint-format-check-changed: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	@files=$$(git diff --name-only --diff-filter=ACMR upstream/main...HEAD -- 'litellm/*.py' | grep -v '^litellm/enterprise/' || true); \
	if [ -z "$$files" ]; then \
...
lint-basedpyright: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	$(UV_RUN) python scripts/type_check_gate.py --base upstream/main
...
lint-type-discipline: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	$(UV_RUN) python scripts/type_discipline_gate.py --base upstream/main
...
lint-test-quality: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	$(UV_RUN) python scripts/test_quality_gate.py --base upstream/main
...
lint-gate: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	$(UV_RUN) python scripts/ruff_strict_gate.py --base upstream/main
...
# does (merge-base with upstream/main). Setup (env sync, Prisma client,
