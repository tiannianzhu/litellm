...
class Router:
    ...
    async def async_get_vllm_queue_metrics(self, use_cache: bool = True) -> Mapping[str, VLLMDeploymentMetrics]:
        ...
        for deployment in deployments:
            validated_deployment = _validated_string_object_dict(deployment)
            if validated_deployment is None:
                continue
            model_info = _validated_string_object_dict(validated_deployment.get("model_info"))
            model_id = model_info.get("id") if model_info is not None else None
