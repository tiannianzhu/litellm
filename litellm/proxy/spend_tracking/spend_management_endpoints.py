...
async def _resolve_request_response_payload(
    ...
    cold_audit: Final = classifier_audit_fields(normalized_payload)
    normalized_psr: Final = _normalize_spend_log_field(normalized_payload.get("proxy_server_request"))
    base_request: Final = normalized_psr if _spend_log_field_has_content(normalized_psr) else stored_request
    resolved_request: Final = (
        {
            **(classifier_input_snapshot(normalized_psr) or stored_request or EMPTY_MAPPING),
            **cold_audit,
        }
        if cold_audit
        else base_request
    ...
    if cold_audit or _spend_log_field_has_content(resolved_request):
        return RequestResponsePayload(
            messages=_normalize_spend_log_field(normalized_payload.get("messages")),
            response=_normalize_spend_log_field(normalized_payload.get("response")),
