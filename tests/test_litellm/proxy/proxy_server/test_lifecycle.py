...
async def test_shutdown_waits_for_blocked_global_logging_callback(monkeypatch):
    ...
    async def stop_jobs(scheduler, executor):
        assert calls == ["callback", "producer"]
        calls.append("scheduler")

    async def drain_counters():
        assert calls == ["callback", "producer", "scheduler"]
        calls.append("counters")

    async def drain_spend_logs():
        assert calls == ["callback", "producer", "scheduler", "counters"]
        calls.append("spend_logs")

    async def drain_s3():
        assert calls == ["callback", "producer", "scheduler", "counters", "spend_logs"]
        calls.append("s3")

    monkeypatch.setattr(ps, "scheduler", object())
    monkeypatch.setattr(ps, "scheduler_executor", object())
    monkeypatch.setattr(ps, "stop_in_flight_scheduler_jobs", stop_jobs)
    ...
    assert calls == ["callback", "producer", "scheduler", "counters", "spend_logs", "s3"]
