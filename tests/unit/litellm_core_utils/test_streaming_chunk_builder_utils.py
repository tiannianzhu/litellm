...
@pytest.mark.parametrize(("provider_total", "provider_reasoning"), [(0, 0), (100, 40)])
def test_usage_prefers_provider_reasoning_detail_over_local_estimate(
    provider_total: int, provider_reasoning: int
) -> None:
    from litellm.types.utils import CompletionTokensDetailsWrapper

    details: Final = CompletionTokensDetailsWrapper(reasoning_tokens=provider_reasoning)
    chunk: Final = ModelResponseStream(
        model="glm-5.3-flash",
        choices=[],
    ...
    assert usage.completion_tokens == provider_total
    assert usage.completion_tokens_details.reasoning_tokens == provider_reasoning
